...
Curb regulations always apply to a linear segment of the street edge. Therefore, CurbLR requires location information to be stored as street segments: features that refer to a particular street and direction of travel, and which have a start and an end point. This enables producers and consumers to communicate clearly about where a restriction applies.
...
1. _By determining relationships between signs_. Regulations are often indicated by multiple signs along the same street. With this method, each input data point is assigned a value to indicate whether it is the beginning (`1`), middle (`2`), or end (`3`) of a regulation. The processing tools examine each individual street to identify points that describe the same regulation, then use the point relationships to turn these points into a street segment. This method is the preferred, most precise method and it is recommended for any new data collection.
2. _A variable-width buffer_, where each point in the input data has a specified width. If a city only has point-based data, then the location of the regulation must be extrapolated/estimated. Certain assets, like parking meters, often have a typical length. The approximate length of each feature can be set in the input data and used to generate a street segment. Overlapping street segments with the same regulation information will be combined into one street segment.
3. _A fixed-width buffer_, where all points are assigned the same length. This is the least common scenario and should only be used if appropriate.
4. _A combination of all three methods_, determined by how much data is available for each point. Sign relationships would be given priority, followed by fixed-buffer width, followed by a fallback buffer size.
