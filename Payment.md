...
| methods | array of `string` Well-known values: `meter`, `pay station`, `digital`, `phone` | How the fee is collected at the street-level, including through digital payment. | `["pay station", "digital"]` |
| forms | array of `string` Well-known values: `coins`, `bills`, `Visa`, `ApplePay`, `MasterCard`, `Visa`, `American Express`, `Discover`, `AndroidPay`. These are not exhaustive. Additional, locally-specific values exist. | What type of payment is accepted. Expressed as an array. | `["coins", "SFMARTA card", "ParkingApp"]` |
